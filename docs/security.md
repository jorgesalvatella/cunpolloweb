...
Vercel Edge (middleware next-intl)
  ...
  |
  |-- POST /api/webhooks/openpay (TODO)
...
- **Nunca** exponer `SUPABASE_SERVICE_ROLE_KEY` o llaves privadas de pago en variables `NEXT_PUBLIC_`
...
- El webhook de pago es un safety net — el flujo principal actualiza status en el mismo POST de `/api/orders`
