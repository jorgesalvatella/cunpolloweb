...
export default function PWARegister() {
  ...
  const [showIosSafari, setShowIosSafari] = useState(false);
  const [showIosOther, setShowIosOther] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isIos()) {
      if (isIosSafari()) {
        setShowIosSafari(true);
      } else {
        setShowIosOther(true);
      }
      return;
    }
  ...
  const dismiss = () => {
    ...
    setShowIosSafari(false);
    setShowIosOther(false);
  };
  ...
  // iOS Chrome/Firefox/etc: redirect to Safari
  if (showIosOther) {
    return (
      <div className="fixed bottom-4 left-4 right-4 z-50 sm:left-auto sm:right-4 sm:w-96">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-5 relative">
